...
wait_for_play_again_keypress:
    IN 5                         ; read the keyboard status 
    CMP A, 0                     ; has anything happened? 
    JE wait_for_play_again_keypress ; if not, read the keyboard status again 
    IN 6                         ; read the key code
    MOV B, A                     ; store the key code in B
    CMP B, 'r'                   ; check if the key is 'r'
    JE clear_screen_and_reset    ; if 'r' is pressed, clear the screen and restart the game
    JMP wait_for_play_again_keypress ; otherwise, keep waiting
