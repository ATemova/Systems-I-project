...
play_again1: DB " Press 'r' to\x00"
play_again2: DB "  play again\x00"
;===============================================================================================|
...
return_string_score:             ; start of the return_string_score routine
    ...
    MOVB [SCORE], BL             ; move the byte in BL back to SCORE
...
wait_loop_for_choice2:
    IN 5                         ; read the keyboard status
    CMP A, 0                     ; compare the status with 0
    JNE end_wait_loop_for_choice2; if not equal, jump to end_wait_loop_for_choice2
    JMP wait_loop_for_choice2    ; otherwise, keep waiting

end_wait_loop_for_choice2:
    IN 6                         ; read the key code
    CMP A, 'f'                   ; compare the key code with 'f'
    JE print2_score              ; if equal, jump to print2_score
    CMP A, 's'                   ; compare the key code with 's'
    JE incre_score               ; if equal, jump to incre_score
...
; Display play again prompt after Level 3
MOV D, 0x0702                	 ; position for play again message
MOV B, play_again1           	 ; string
CALL draw_text               	 ; display play again message
MOV D, 0x0802                	 ; position for play again message
MOV B, play_again2           	 ; string
CALL draw_text               	 ; display play again message

wait_for_play_again:
    IN 5                         ; read the keyboard status 
    CMP A, 0                     ; has anything happened? 
    JE wait_for_play_again		 ; if not, read the keyboard status again 
    IN 6                         ; read the key code
    MOV B, A                     ; store the key code in B
    CMP B, 'r'                   ; check if the key is 'r'
    JE reset    				 ; if 'r' is pressed, clear the screen and restart the game
    JMP wait_for_play_again		 ; otherwise, keep waiting

reset:
    MOV A, 3                     ; load 3 into register A
    OUT 7                        ; clear screen
    MOVB [SCORE], 0x30           ; reset the score to '0'
